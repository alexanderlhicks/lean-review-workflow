...
from lean_utils import file_path_to_module_name, is_in_comment
...
def extract_axioms(module_name: str, declarations: List[str]) -> Dict[str, List[str]]:
    ...
    for decl in declarations:
        ...
        # Parse output: lines like "  'axiomName' depends on axioms: [ax1, ax2]"
        # or "'declName' does not depend on any axioms"
        axioms = []
        if "does not depend on any axioms" in output:
            axiom_map[decl] = []
        else:
            # Extract axiom names from lines
            for line in output.splitlines():
                line = line.strip()
                if line and not line.startswith('#') and not line.startswith('--'):
                    # Each line in #print axioms output is an axiom name
                    if line not in ('', module_name):
                        axioms.append(line)
            if axioms:
